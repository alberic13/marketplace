            ...
            <form action="{{ route('listings.update', $listing) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <select name="game_id" id="game_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach($games as $game)
                            <option value="{{ $game->id }}" {{ (old('game_id') ?? $listing->game_id) == $game->id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="4" required 
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Describe your listing">{{ old('description') ?? $listing->description }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            <option value="sell" {{ (old('type') ?? $listing->type) == 'sell' ? 'selected' : '' }}>Selling</option>
                            <option value="buy" {{ (old('type') ?? $listing->type) == 'buy' ? 'selected' : '' }}>Looking to Buy</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="condition" id="condition" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            <option value="digital" {{ (old('condition') ?? $listing->condition) == 'digital' ? 'selected' : '' }}>Digital</option>
                        </select>
                        @error('condition')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price') ?? $listing->price }}" required min="0" step="0.01"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                           placeholder="0.00">
                    @error('price')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="game_key" id="game_key" value="{{ old('game_key') ?? $listing->game_key }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Any additional information">{{ old('notes') ?? $listing->notes }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-between">
                    <a href="{{ route('listings.show', $listing) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded-lg">
                        Cancel
                    </a>
